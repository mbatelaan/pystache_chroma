...
file = open('unperturbed_prop_template.xml.ms', 'r').read()
...
open('unperturbed_prop_input.xml', 'w+').write(render_ms(template, parameters).replace('[[', '{{').replace(']]', '}}'))

file = open('perturbed_prop_q440_template.xml.ms', 'r').read()
template = pystache.parse(file)
ms_renderer = pystache.Renderer(partials = partials)
def render_ms(template, obj):
    return ms_renderer.render(template,obj)
open('perturbed_prop_q440_input.xml', 'w+').write(render_ms(template, parameters).replace('[[', '{{').replace(']]', '}}'))

file = open('perturbed_prop_q420_template.xml.ms', 'r').read()
template = pystache.parse(file)
ms_renderer = pystache.Renderer(partials = partials)
def render_ms(template, obj):
    return ms_renderer.render(template,obj)
open('perturbed_prop_q420_input.xml', 'w+').write(render_ms(template, parameters).replace('[[', '{{').replace(']]', '}}'))

file = open('perturbed_prop_q222_template.xml.ms', 'r').read()
template = pystache.parse(file)
ms_renderer = pystache.Renderer(partials = partials)
def render_ms(template, obj):
    return ms_renderer.render(template,obj)
open('perturbed_prop_q222_input.xml', 'w+').write(render_ms(template, parameters).replace('[[', '{{').replace(']]', '}}'))

file = open('perturbed_prop_q200_template.xml.ms', 'r').read()
template = pystache.parse(file)
ms_renderer = pystache.Renderer(partials = partials)
def render_ms(template, obj):
    return ms_renderer.render(template,obj)
open('perturbed_prop_q200_input.xml', 'w+').write(render_ms(template, parameters).replace('[[', '{{').replace(']]', '}}'))
